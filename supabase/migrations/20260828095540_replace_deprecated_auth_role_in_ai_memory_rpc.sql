-- Keep tenant-aware AI memory RPCs compatible with current Supabase auth guidance.
-- auth.role() is deprecated; the signed JWT role claim is sufficient for the
-- intentional service-role bypass while authenticated users must pass workspace membership.
...
begin
  ...
  if not coalesce((select auth.jwt() ->> 'role') = 'service_role', false)
